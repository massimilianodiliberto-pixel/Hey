/* ============================================================================
   Vendored, unmodified, from https://github.com/oso95/scroll-world (MIT License,
   Copyright (c) 2026 cyw). Only the runtime engine is used here — not that
   project's Higgsfield/Monid generation pipeline. See LICENSE-scroll-world.txt.
   ============================================================================ */
/* ============================================================================
...
function mountScrollWorld(container, config) {
  ...
  SECTIONS.forEach((s, i) => {
    const dive = { kind: 'dive', si: i, clip: s.clip, clipM: s.clipMobile, still: s.still, stillM: s.stillMobile,
                   accent: s.accent, w: s.scroll || DIVE_W, linger: s.linger || 0, loopIdle: !!s.loopIdle };
    SEGMENTS.push(dive);
    s._seg = dive;
  ...
  function raf() {
    ...
    // Local patch (HEY, not upstream): a dive section flagged `loopIdle` (only
    // section 0 in practice — the pre-scroll "HEY." loop) free-plays on a native
    // loop instead of sitting scrub-controlled at frame 0, for as long as the
    // page hasn't been scrolled at all. The instant the user scrolls, control
    // reverts to normal currentTime scrubbing like every other segment — this
    // never re-engages even if they scroll back to the very top, so the loop
    // can't stutter in and out at the boundary.
    const scrolled = (window.scrollY || window.pageYOffset) > 2;
    for (let i = 0; i < NSEG; i++) {
      const s = SEGMENTS[i];
      ...
      if (s.loopIdle) {
        if (!scrolled) {
          ...
          continue; // free-playing: never force currentTime while idle
        } else if (s.video.loop) {
          s.video.loop = false;
  ...
  //
  // Local patch (HEY, not upstream): the original only primed on touch/pointerdown,
  // gated to isMobile(). In practice a seeked-but-never-played muted <video> can also
  // paint blank on desktop (no compositor frame yet) if the very first user gesture is
  // a wheel scroll rather than a pointerdown — which is the common case on desktop.
  // Priming is a no-op cost (instant muted play→pause), so it's safe to always run it,
  // on any input device, on the first gesture of any kind.
  let userReady = false;
